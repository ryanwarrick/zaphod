...
* Dev-Ops
  * zaphod_ansible (separate project):
    ...
    * For more info, see the zaphod_ansible [write-up](http://ryanwarrick.com/career/projects/zaphod_ansible) on the website or check out my [zaphod_ansible](https://github.com/ryanwarrick/zaphod_ansible) GitHub repo.
  * utils/utils.py:
...
# App Content Data
Zaphod is designed so one can easily write content (.md or .html files) for the website, and then simply place them in the appropriate directory within the instance folder for consumption and presentation by the web app.
...
Once created, you can now load the directory with content for Zaphod to use and display. Zaphod expects the following structure (note the content and config files):
```
INSTANCE_DIR_PATH:
|   config.py (not required since default values are set, but strongly encouraged)
|   zaphod.sqlite (auto-generated by app per content of 'content' directory - see below)
|___content
    |__articles
    |  |__Topic1
    |  |  |__SubtopicA
